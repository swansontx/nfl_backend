"""Train Team Score Last model.

Team to Score Last is a popular DraftKings prop (typically -110 for both teams).
We use play-by-play data to identify which team scored last and model the probability
based on closing drive tendencies, offensive efficiency, and game script.

Expected Performance: 55-60% hit rate, +10-20% ROI (small edge over 50%)

Features:
- Historical score last rate (season & L3)
- Fourth quarter scoring rate
- Trailing team tendencies
...
def extract_last_score_data(pbp_df):
    """Extract which team scored last in each game.

    Args:
        pbp_df: Play-by-play DataFrame

    Returns:
        DataFrame with last scoring team per game
    """

    print(f"\n{'='*80}")
    print(f"EXTRACTING LAST SCORE DATA")
    print(f"{'='*80}\n")
    ...
    scoring_plays = scoring_plays.sort_values(['game_id', 'game_seconds_remaining'], ascending=[True, True])

    # Get LAST score of each game
    last_scores = scoring_plays.groupby('game_id').first().reset_index()

    print(f"Games with scores: {len(last_scores)}")
    print()

    # Show distribution
    if 'posteam' in last_scores.columns:
        print("Last scoring teams distribution:")
        print(f"  Total games: {len(last_scores)}")
        print()

    return last_scores[['game_id', 'posteam', 'home_team', 'away_team', 'week']]
...
def create_team_game_data(pbp_df):
    ...
    - Did they score last? (target)
    - Historical score last rate
    - Fourth quarter stats
    - Closing drive efficiency
    - Pace metrics
    ...
    # Add scored last indicator
    last_scores = extract_last_score_data(pbp_df)[['game_id', 'posteam']]
    last_scores.columns = ['game_id', 'team']
    last_scores['scored_last'] = 1

    team_games = team_games.merge(last_scores, on=['game_id', 'team'], how='left')
    team_games['scored_last'] = team_games['scored_last'].fillna(0).astype(int)

    print(f"Teams that scored last: {team_games['scored_last'].sum()}")
    print(f"Score last rate: {team_games['scored_last'].mean():.1%}")
    print()
...
def create_score_last_features(team_games_df):
    """Create features for score last model.
    ...
    print(f"\n{'='*80}")
    print(f"CREATING SCORE LAST FEATURES")
    print(f"{'='*80}\n")
    ...
    df['season_score_last_rate'] = df.groupby('team')['scored_last'].expanding().mean().reset_index(level=0, drop=True)
    ...
    # L3 stats
    df['l3_score_last_rate'] = df.groupby('team')['scored_last'].rolling(window=3, min_periods=1).mean().reset_index(level=0, drop=True)
    ...
    print(f"Score last rate: {df['scored_last'].mean():.2%}")
    print()

    return df


def train_score_last_model(train_df):
    """Train score last model.
    ...
    print(f"\n{'='*80}")
    print(f"TRAINING SCORE LAST MODEL")
    print(f"{'='*80}\n")

    # Features
    feature_cols = [
        'season_score_last_rate',
        'l3_score_last_rate',
...
def backtest_score_last(model, features, team_games_df, pbp_df, week):
    """Backtest score last model on a specific week.
    ...
    print(f"\n{'='*80}")
    print(f"BACKTESTING SCORE LAST - WEEK {week}")
    print(f"{'='*80}\n")
    ...
    team_stats.columns = ['team', 'season_score_last_rate', 'season_opening_drive_rate',
                          'season_q1_points_avg', 'season_plays_per_drive', 'games_played']

    # L3 stats
    historical_df = historical_df.sort_values(['team', 'week'])

    l3_score = historical_df.groupby('team')['scored_last'].rolling(window=3, min_periods=1).mean().reset_index(level=0, drop=True)
    historical_df['l3_score_last_rate'] = l3_score
    ...
    team_l3 = historical_df.groupby('team')[['l3_score_last_rate', 'l3_opening_drive_rate', 'l3_q1_points_avg']].last().reset_index()
    ...
    prob_score_last = model.predict_proba(X)[:, 1]
    week_df['prob_score_last'] = prob_score_last

    # Calculate edge (typical odds -110 = 52.4% implied)
    week_df['edge'] = week_df['prob_score_last'] - 0.524
    ...
    if len(bets) > 0:
        ...
        top_bets = bets.nlargest(10, 'prob_score_last')[['team', 'is_home', 'prob_score_last', 'edge', 'scored_last']]
        for idx, row in top_bets.iterrows():
            status = '✅' if row['scored_last'] else '❌'
            home_away = 'HOME' if row['is_home'] else 'AWAY'
            print(f"  {status} {row['team']:5s} ({home_away}): Prob={row['prob_score_last']:.1%}, Edge={row['edge']:+.1%}")
...
def main():
    """Main function to train score last model."""

    print(f"\n{'='*80}")
    print(f"TEAM SCORE LAST MODEL TRAINING")
    print(f"{'='*80}\n")
    ...
    # Create features
    team_games_features = create_score_last_features(team_games)
    ...
    # Train model
    model, features, metrics = train_score_last_model(train_df)

    # Save model
    models_dir = Path('outputs/models/pbp')
    models_dir.mkdir(parents=True, exist_ok=True)

    model_file = models_dir / 'team_score_last_model.pkl'
    joblib.dump({
        'model': model,
        'features': features,
        'config': {
    ...
    for week in [10, 11]:
        result = backtest_score_last(model, features, team_games_features, pbp_df, week)
    ...
    # Summary
    print(f"\n{'='*80}")
    print(f"TEAM SCORE LAST TRAINING COMPLETE")
    print(f"{'='*80}\n")
    ...
    print("✅ Team Score Last model trained and backtested")
    print()
    print("NEW MARKET AVAILABLE:")
    print("  ✅ Team to Score Last")
    print()
